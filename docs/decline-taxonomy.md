...
| `authentication_failed` | `AUTH_REQUIRED_AFA` | `RETRY_FUTILE` | unverified |
| `payment_cancelled` | see §6 | — | unverified |
| `payment_timed_out` | see §7 | — | no |
| `payment_collect_request_expired` | — | — | no |
| `incorrect_cvv` | — | — | no |
...
*and* the current subscription/mandate state. A classifier reading only `reason` cannot
...
exceeded the time limit, *or* there was partner bank downtime. Opposite implications: one
